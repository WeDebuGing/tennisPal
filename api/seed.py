...
with app.app_context():
    ...
    users = []
    for i in range(20):
        u = User(name=fake.name(), email=fake.unique.email(), phone=fake.unique.numerify('###-###-####'),
                 password_hash=pw, ntrp=NTRP_LEVELS[i], elo=random.randint(1000, 1600))
        db.session.add(u)
        users.append(u)
    db.session.flush()
    ...
    for u in users:
        for _ in range(random.randint(1, 3)):
            ...
            db.session.add(Availability(user_id=u.id, day_of_week=day,
    ...
    for i in range(8):
        ...
        db.session.add(LookingToPlay(
            user_id=u.id, play_date=d, start_time=f"{h:02d}:00", end_time=f"{h+2:02d}:00",
            court=random.choice(COURTS), match_type=random.choice(['singles', 'doubles', 'hitting']),
            level_min=u.ntrp - 0.5 if u.ntrp else None, level_max=u.ntrp + 0.5 if u.ntrp else None))
    ...
    for _ in range(30):
        ...
        db.session.add(Match(player1_id=p1.id, player2_id=p2.id, play_date=d, match_type='singles',
                             status='completed', score=scores, score_submitted_by=p1.id,
                             score_confirmed=True, winner_id=winner.id))
